...
output:
  xaringan::moon_reader:
    css: ["default.css", "mySlides.css", "style.css", "animate.xaringan.css", "rutgers-fonts.css"]
    lib_dir: libs
    nature:
...
name: title
class: left, middle
background-image: url(img/logo-cnrs.png), url(img/iker_sinadura.png), url(img/logo-lmu.png), url(img/logo-ehu-gardena-2.png), url(img/logo-uppa.jpg), url(img/icphs2023.png)
background-position: 28% 94%, 40% 94%, 60% 94%, 78% 94%, 95% 94%, 90% 53%
background-size: 8%, 15%, 15%, 17%, 15%, 50%
...
### By category
```{r}
as.data.frame(hdata.m.new_sp_encoding %>% dplyr::count(etym_asp))
```
]
...
### By speaker
```{r}
as.data.frame(hdata.m.new_sp_encoding %>% dplyr::group_by(speaker_new) %>% dplyr::count(etym_asp) %>% tidyr::pivot_wider(names_from = etym_asp, values_from = n) %>% dplyr::rename(speaker = speaker_new))
```
]
...
There is an opposition between oral and nasalized aspirates!
