...
name: title
class: left, middle
background-image: url(img/logo-cnrs.png), url(img/logo-iker.png), url(img/logo-lmu.png), url(img/logo-ehu-gardena-2.png), url(img/logo-uppa.jpg), url(img/icphs2023.png)
background-position: 22% 93%, 38% 93%, 60% 93%, 78% 93%, 95% 93%, 90% 50%
background-size: 8%, 18%, 15%, 17%, 15%, 50%
background-color: darkgrey

# A nasalance-based study of the /h/ vs. /h̃/ opposition in Zuberoan Basque

.oneline[Ander Egurtzegi

.extrasmall[ander.egurtzegi@iker.cnrs.fr]]

.oneline[Andrea García-Covelo

.extrasmall[andrea.garcia@phonetik.uni-muenchen.de]]

.oneline[Iñigo Urrestarazu-Porta

.extrasmall[inigo.urrestarazu-porta@iker.cnrs.fr]]

.fancy[Prage | `r Sys.Date()`]
...
# Previous literature on Basque nasalized aspirates
...
<br>
<br>
One acoustic analysis of h vs h̃ (Egurtzegi & Carignan 2020)

A handful of illustrative spectrograms (Egurtzegi 2023) 

]

<div style = "position:fixed; left: 30px; bottom:30px; right:50px; font-size:80%; text-align:left">
...
<br>
Egurtzegi, Ander. 2018. On the phonemic status of nasalized /h/ in Modern Zuberoan Basque. <em>Linguistics</em> 56. 1353-1367. doi: <a href="https://doi.org/10.1515/ling-2018-0024">10.1515/ling-2018-0024</a>.
<br>
Egurtzegi, Ander & Carignan, Christopher. 2020. A typological rarity: the /h/ vs. /h/ contrast of Mixean Basque. <em>Labphon</em> 17.
<br>
Egurtzegi, Ander. 2023. /h/ hasperen sudurkarituaren inguran [On the nasalized aspiration /h/]. <em>International Journal of Basque Linguistics and Philology</em> 57.
</div>
...
Laryngeal loss
<br>
only in post-tonic last syllable
<br>
...
Is nasalization os aspirates still found in Zuberoan variaties of Basque 
--
(> Larraine Basque)?

--

If yes, let's describe it

If yes, how about younger generations?
...
load("../data/nasl.mod.RData")
load("../data/nasM.mod.RData")
nasl.mod$formula
nasM.mod$formula
```
...
## Bayesian generalized mixed-effects model: further specs
...
library(leaflet)
leaflet() %>% 
  setView(lat = 43.01889273366674, lng = -0.9556880607342878, zoom = 3) %>% addProviderTiles(providers$OpenStreetMap)
```

---

## Stimuli: wordlist
| Etymological /h/                | Etymological /h̃/           | Assimilated                                   |
|----------------------|-------------------|-------------------------------|
| *zahar* 'old', *behi* 'cow' | *ahate* 'duck', *uhue* 'honor' | *nihaur* 'myself', *lehen* 'before', *uñhu* 'onion' |
